...
(comment
  ...
  (-> (clara-rules/analyze-defquery-macro {:node query-node}) :node api/sexpr)

  (-> (clara-rules/analyze-defrule-macro {:node rule-node}) :node api/sexpr)
  ...
  (-> (clara-rules/analyze-defrule-macro
       {:node (api/parse-string "(defrule unmatch-borrower-credit-report-context-singleton-rule
                                    ...
                                    =>
                                    (run! retract! ?context-seq))")}) :node api/sexpr)
